/*******************************************************************************
SCRIPT NAME         lego_secure_job.sql 
 
LEGO OBJECT NAME    LEGO_SECURE_JOB
...
DECLARE
  ...
  v_source           VARCHAR2(64) := 'lego_secure_job.sql';
  v_lego_object_name VARCHAR2(64) := 'LEGO_SECURE_JOB'; 
 ...
 WHERE fw.firm_worker_id IN
   ...
   AND NVL(job.archived_date,SYSDATE) >= ADD_MONTHS(TRUNC(SYSDATE),- months_in_refresh)  -- Time-based limit. The string
 UNION      -- bottom half uses different joins to get CAM.                              -- "months_in_refresh" will be replaced at runtime.
