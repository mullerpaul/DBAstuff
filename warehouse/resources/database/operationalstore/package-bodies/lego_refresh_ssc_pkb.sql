CREATE OR REPLACE PACKAGE BODY lego_refresh_supp_scorecard AS

    gc_logging_source                CONSTANT VARCHAR2(27) := 'LEGO_REFRESH_SUPP_SCORECARD';
    ...
    BEGIN
        ...
        logger_pkg.set_code_location('supplier scorecard');
        ...
        logger_pkg.info('starting lego refreshes for supplier_scorecard');
        ...
        lego_tools.insert_history_parent_row(
            ...
            pi_caller_id           => 'Supplier Scorecard',
        ...
        FOR lego IN (
            WITH top_level_legos   --entries for the "top-level" lego(s)
              AS ( SELECT object_name,
                          source_name
                     FROM lego_refresh
                    WHERE object_name = 'LEGO_SUPPLIER_SCORECARD'
