DECLARE
  lv_source     processing_log.source%TYPE := 'msvc-3258_get_historical_data_ut01.sql';
  lv_msg        VARCHAR2(255);
...
BEGIN
  ...
  logger_pkg.set_code_location('msvc-3258_get_historical_data_ut01');
...
-- delete data
  ...
  IF 
    ...
    l_cursor1%rowcount <> 0 
  THEN
    lv_msg := 'msvc-3258_get_historical_data_ut01 FAILED.';
    logger_pkg.info(lv_msg, TRUE);
    raise_application_error(-20001, lv_msg);

  END IF;
  
  lv_msg := 'msvc-3258_get_historical_data_ut01 PASSED.';
  logger_pkg.info(lv_msg,TRUE);
