DECLARE
  lv_source     processing_log.source%TYPE := 'msvc-2892_get_api_ut01.sql';
  lv_msg        VARCHAR2(255);
...
BEGIN
  ...
  logger_pkg.set_code_location('msvc-2892_get_api_ut01');
...
-- delete data
  ...
  IF (lv_cat_coeff_old <> lv_cat_coeff_oldr OR
    ...
    lv_msg := 'msvc-2892_get_api_ut01 FAILED.';
    logger_pkg.info(lv_msg, TRUE);
    raise_application_error(-20001, lv_msg);
  END IF;
  
  lv_msg := 'msvc-2892_get_api_ut01 PASSED.';
  logger_pkg.info(lv_msg,TRUE);
