DECLARE
    lv_source     processing_log.source%TYPE := 'msvc-2694_client_metric_settings_api_call_ut02.sql';
    lv_msg                 VARCHAR2(255);
    lc_dummy_client_guid   CONSTANT RAW(16) := hextoraw('ABCD000000000000000000000000ABCD');
    lv_txn_log_pk          RAW(16) := sys_guid;
    lv_ccc_pk              RAW(16) := sys_guid;
...
BEGIN
  ...
  logger_pkg.set_code_location('msvc-2694_client_metric_settings_api_call_ut02');
  
   
    INSERT INTO client_visibility_list ( log_in_client_guid, 
    visible_client_guid,
    score_config_owner_guid ) 
    VALUES ( lc_dummy_client_guid,
...
-- remove data
    ...
    DELETE FROM client_visibility_list WHERE
        log_in_client_guid = lc_dummy_client_guid;

    DELETE FROM client_category_coefficient WHERE
        client_guid = lc_dummy_client_guid;

    DELETE FROM transaction_log WHERE
        bus_org_guid = lc_dummy_client_guid;

    COMMIT;
	
    IF lv_new_cat_coeff <> 5 THEN
    lv_msg := 'msvc-2694_client_metric_settings_api_call_ut02 FAILED.';
    logger_pkg.info(lv_msg);
    raise_application_error(-20001, lv_msg);
	
  END IF;
  
  lv_msg := 'msvc-2694_client_metric_settings_api_call_ut02 PASSED.';
  logger_pkg.info(lv_msg);
