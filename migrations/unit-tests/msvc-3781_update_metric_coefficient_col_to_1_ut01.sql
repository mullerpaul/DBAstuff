DECLARE
  lv_source     processing_log.source%TYPE := 'msvc-3781_update_metric_coefficient_col_to_1_ut01.sql';
  lv_msg        VARCHAR2(255);
...
BEGIN
  ...
  logger_pkg.set_code_location('msvc-3781_update_metric_coefficient_col_to_1_ut01');
  ...
  logger_pkg.info('Checking update on metric_coefficient column...');
	...
	WHERE METRIC_COEFFICIENT <> 1; 
	
  IF lv_check_not_updated_data <> 0 THEN
    lv_msg := 'msvc-3781_update_metric_coefficient_col_to_1_ut01 FAILED.';
    logger_pkg.info(lv_msg, TRUE);
    raise_application_error(-20001, lv_msg);
  END IF;
  
  lv_msg := 'msvc-3781_update_metric_coefficient_col_to_1_ut01 PASSED.';
  logger_pkg.info(lv_msg,TRUE);
