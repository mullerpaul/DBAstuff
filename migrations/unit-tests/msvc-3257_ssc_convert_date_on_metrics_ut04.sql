DECLARE
  lv_source     processing_log.source%TYPE := 'msvc-3257_ssc_convert_date_on_metrics_ut04.sql';
  lv_msg        VARCHAR2(255);
...
BEGIN
  ...
  logger_pkg.set_code_location('msvc-3257_ssc_convert_date_on_metrics_ut04');
...
-- delete data
  ...
  THEN
    lv_msg := 'msvc-3257_ssc_convert_date_on_metrics_ut04 FAILED.';
    logger_pkg.info(lv_msg, TRUE);
    raise_application_error(-20001, lv_msg);

  END IF;
  
  lv_msg := 'msvc-3257_ssc_convert_date_on_metrics_ut04 PASSED.';
  logger_pkg.info(lv_msg,TRUE);
