DECLARE
  lv_source     processing_log.source%TYPE := 'msvc-2692_audit_table_client_metric_conversion_ut01.sql';
  lv_msg        VARCHAR2(255);
...
BEGIN
  ...
  logger_pkg.set_code_location('msvc-2692_audit_table_client_metric_conversion_ut01');
  
  logger_pkg.info('Checking for audit columns');
  ...
  IF (lv_row_count_effective_date <> 1 OR
    ...
    lv_msg := 'msvc-2692_audit_table_client_metric_conversion_ut01 FAILED.';
    logger_pkg.info(lv_msg, TRUE);
    raise_application_error(-20001, lv_msg);

  END IF;
  
  lv_msg := 'msvc-2692_audit_table_client_metric_conversion_ut01 PASSED.';
  logger_pkg.info(lv_msg,TRUE);
