DECLARE
  lv_source     processing_log.source%TYPE := 'msvc-2892_get_api_ut04.sql';
  lv_msg        VARCHAR2(255);
...
BEGIN
  ...
  logger_pkg.set_code_location('msvc-2892_get_api_ut04');
 ...
 -- get the existing data here       
    ...
    li_get_datetime_old := sys_extract_utc(systimestamp);    
...
-- call SET here      
    ...
    li_get_datetime_new := sys_extract_utc(systimestamp) + 3;                                                
...
-- delete data
  ...
  IF (  lv_score_a_old <> lv_score_a_oldr OR
    ...
    lv_msg := 'msvc-2892_get_api_ut04 FAILED.';
    logger_pkg.info(lv_msg, TRUE);
    raise_application_error(-20001, lv_msg);
  END IF;
  
  lv_msg := 'msvc-2892_get_api_ut04 PASSED.';
  logger_pkg.info(lv_msg,TRUE);
